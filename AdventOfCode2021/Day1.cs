namespace AdventOfCode2021
{
    class Day1
    {
        ...
        {
            List<int> list = File.ReadAllLines(path).Select(x => int.Parse(x)).ToList();            
            Console.WriteLine(Enumerable.Range(1, list.Count - 2).Where(i => list.Count > i + 1 && list.Count > i + 2 && list[i] + list[i +1] + list[i + 2] > list[i-1] + list[i] + list[i +1]).ToList().Count);
